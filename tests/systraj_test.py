...
def test_uniform_flag():
    ...
    smooth = [3, 4]
    degree = [5, 6]
    basis = ntg.BSplineFamily(breakpoints, degree, smooth, nout)
    ...
    for i in range(nout):
        ztraj[i, 0:flaglen[i]] = np.array([
            ntg.spline_interp(t, np.array(breakpoints), ninterv, coef_list[i],
                              degree[i] + 1, smooth[i], flaglen[i])
