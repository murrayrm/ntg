...
int *nstate)
{
	...
	if(Gnnltc!=0)
		updateZ(GZ,Gccolloc,x,Gtrajectoryconstrav,Gntrajectoryconstrav,AVTRAJECTORY);
	if(Gnnlfc!=0)
	...
	switch(*mode)
	{
		case 0: /* only compute constraint functions */
			NonLinearConstraints(mode,nstate,C,NULL,
				Gnnlic,Gnlicf,Gnnltc,Gnltcf,Gnnlfc,Gnlfcf,Gccolloc,GZ);
			break;
		case 1: /* only compute constraint gradient */
			NonLinearConstraints(mode,nstate,NULL,GcJac,
				Gnnlic,Gnlicf,Gnnltc,Gnltcf,Gnnlfc,Gnlfcf,Gccolloc,GZ);
			break;
		case 2: /* compute both constraint function and gradients */
